...
TTT_THRESHOLDS = {
    "Critical": float(os.getenv("TTT_HOURS_CRITICAL")),
    "High": float(os.getenv("TTT_HOURS_HIGH")),
    "Medium": float(os.getenv("TTT_HOURS_MEDIUM")),
    "Low": float(os.getenv("TTT_HOURS_LOW"))
}
...
def get_next_business_hour(dt):
    if is_business_hours(dt):
        return dt
    
    if dt.weekday() in BUSINESS_DAYS and dt.hour < BUSINESS_START_HOUR:
        return dt.replace(hour=BUSINESS_START_HOUR, minute=0, second=0, microsecond=0)
    
    days_ahead = 0
    while True:
        next_day = dt + timedelta(days=days_ahead + 1)
        if next_day.weekday() in BUSINESS_DAYS:
            return next_day.replace(hour=BUSINESS_START_HOUR, minute=0, second=0, microsecond=0)
        days_ahead += 1

def should_apply_business_hours_constraint(criticality):
    return criticality in ["Low", "Medium"]
...
def calculate_business_hours_between(start_dt, end_dt):
    ...
    while current_dt < end_dt:
        if current_dt.weekday() not in BUSINESS_DAYS:
            current_dt += timedelta(days=1)
            current_dt = current_dt.replace(hour=0)
            continue
        
        day_start = max(current_dt, current_dt.replace(hour=BUSINESS_START_HOUR))
        day_end = min(end_dt, current_dt.replace(hour=BUSINESS_END_HOUR))
        
        if day_start < day_end and day_start.hour < BUSINESS_END_HOUR and day_end.hour > BUSINESS_START_HOUR:
            hours_this_day = (day_end - day_start).total_seconds() / 3600
...
def add_business_hours(start_dt, hours_to_add):
    if hours_to_add <= 0:
        return start_dt
    ...
    while remaining_hours > 0:
        if not is_business_hours(current_dt):
            current_dt = get_next_business_hour(current_dt)
        
        end_of_business_today = current_dt.replace(hour=BUSINESS_END_HOUR, minute=0, second=0, microsecond=0)
        hours_available_today = (end_of_business_today - current_dt).total_seconds() / 3600
        ...
        if remaining_hours <= hours_available_today:
            ...
            current_dt = get_next_business_hour(current_dt + timedelta(days=1))
    
    return current_dt

def calculate_business_hours_reminder_time(start_dt, total_hours, criticality):
    if not should_apply_business_hours_constraint(criticality):
        return start_dt + timedelta(hours=total_hours)
    
    if is_business_hours(start_dt):
        return add_business_hours(start_dt, total_hours)
    else:
...
def send_triage_reminder(ticket):
    if ticket.get('ttt_hours'):
        logging.info(f"TTT already set for ticket #{ticket['id']}, skipping triage reminder.")
        return
    
    try:
        reminder_text = (f"📋 <@{ticket['assignee_id']}>, this ticket needs to be triaged. Please click the Triage button to categorize and provide more details.")
        web_client.chat_postMessage(channel=CHANNEL, thread_ts=ticket['message_ts'], text=reminder_text)
        logging.info(f"Triage reminder sent for ticket #{ticket['id']} to <@{ticket['assignee_id']}>.")
    except Exception as e:
        logging.error(f"Error sending triage reminder: {e}")
    ...
    try:
        ...
        if frt_value:
            ...
            if ticket.get('frt_hours'):
                ...
                ttt_value = TTT_THRESHOLDS.get(ticket['criticality'], '')
                if ttt_value:
                    ttt_text = f"TTT (Time to Triage) for *{ticket['criticality']}* priority is *{ttt_value} hours*. " \
                            f"This is the maximum time allowed to triage this ticket."
                    web_client.chat_postMessage(channel=CHANNEL, thread_ts=ticket['message_ts'], text=ttt_text)

        if not ticket.get('frt_hours'):
            frt_hours = FRT_THRESHOLDS[ticket['criticality']]
            half_frt = max(frt_hours / 2, 0.0167)
            
            reminder_time = calculate_business_hours_reminder_time(now, half_frt, ticket['criticality'])
            ...
            escalation_hours = frt_hours - FRT_ESCALATION_HOURS
            escalation_time = calculate_business_hours_reminder_time(now, escalation_hours, ticket['criticality'])
        ...
        if not ticket.get('ttt_hours'):
            ttt_hours = TTT_THRESHOLDS[ticket['criticality']]
            half_ttt = max(ttt_hours / 2, 0.0167)
            
            triage_reminder_time = calculate_business_hours_reminder_time(now, half_ttt, ticket['criticality'])
            
            scheduler.add_job(send_triage_reminder,trigger=DateTrigger(run_date=triage_reminder_time),args=[ticket],id=f"triage_reminder_{ticket['id']}")
            logging.info(f"Scheduled triage reminder for ticket #{ticket['id']} at {triage_reminder_time}")
        else:
            logging.info(f"TTT already set for ticket #{ticket['id']}, not scheduling triage reminders.")
                ...
                if ticket.get("message_ts") == thread_ts or ticket.get("triage_ts") == thread_ts:
                    ...
                    if (responder_email in escalation_emails or is_assignee) and not ticket.get("frt_hours"):
                        ...
                        if ttt_value:
                            ttt_text = f"TTT (Time to Triage) for *{ticket['criticality']}* priority is *{ttt_value} hours* 📋. " \
                                    f"This is the maximum time allowed to triage this ticket."
                            client.web_client.chat_postMessage(channel=CHANNEL, thread_ts=ticket['message_ts'], text=ttt_text)
                        return
                    else:
        ...
        try:
            ...
            for ticket in all_tickets:
                if ticket["id"] == ticket_id:
                    ...
                    try:
                        scheduler.remove_job(f"triage_reminder_{ticket_id}")
                        logging.info(f"Canceled triage reminder for ticket #{ticket_id}")
                    except Exception as e:
                        logging.info(f"No triage reminder job to remove for ticket #{ticket_id}: {e}")
